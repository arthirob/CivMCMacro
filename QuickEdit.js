...
const damageTreshhold=110; //The damage at which you want to stop using your tool
...
var farmFinished; //Set to true when the farm is finished
...
function checkHeight() { //Throw an error and send a kira message if you feel in the water at some point
    JavaWrapper.methodToJavaAsync(() => {
        while (!farmFinished) {
            Client.waitTick(100);
            if (p.getY()<firstLevel) {
                Chat.log("You feel in the water, cancelling everything and logging you out")
                KeyBind.keyBind("key.jump",false);
                throw("You feel in the water")
            }
        }
      }).run()
