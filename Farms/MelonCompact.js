...
const beaconRange = 49;


//Misc declaration
const compactEveryPlot = 6; // The amount of plot you do before compacting
const lagTick = 6; //Add a little bit of delay when using the lodestone. With better internet, reduce this number to 4 approx
...
const regrowTime = 27;
const pitchValue = 10;
const breakTime=2; //The break time of a melon
...
function walkTo(x, z) { // Walk to the center of a block
    ...
    while ((Math.abs(p.getX() - x - 0.5) > 0.1 || Math.abs(p.getZ() - z - 0.5 ) > 0.1)){
        ...
        Client.waitTick();
        if ((Math.abs((p.getX()-prevX))<0.02)&&(Math.abs((p.getZ()-prevZ))<0.02)) { //You are probably hitting a block, let's destroy it
            stuck++
            KeyBind.keyBind("key.forward", false);
            KeyBind.keyBind("key.attack", true);
            Client.waitTick(stuck*breakTime)
...
function refreshHaste() { //Refresh the haste debuff, only if you are in the northern part, in the range  of the farm
    if (currentPlot<zWestBeacon-(beaconRange-4)) { //Add a -4 so you refresh even when in the line at the limit of the range
        KeyBind.keyBind("key.attack", false);
        currentX = p.getX();
        currentZ = p.getZ();
        if (side==0) {
        ...
        while ((Math.abs(p.getX()-currentX) > 0.1 || Math.abs(p.getZ()-currentZ) > 0.1)) {
...
function goFrontLine() { //Go in front of a line, and attack to clear melon that might be there
    ...
    KeyBind.keyBind("key.attack", false);
    Client.waitTick();
    if ((Math.abs(p.getX() - objX - 0.5) > 0.1 || Math.abs(p.getZ() - objZ - 0.5 ) > 0.1)) { // If for some reason, you are at the wrong spot, move again but slowly
        KeyBind.keyBind("key.sneak", true);
        goFrontLine();
        KeyBind.keyBind("key.sneak", false);
    }
...
function harvestLine() { // Harvest a line of melon
    ...
    while (!lineFinished()) {
        ...
        if (Math.abs((p.getX()-prevX))<0.1) { // This allows to wait if you bump into a melon, to prevent lag
            ...
            KeyBind.keyBind("key.forward", false);
            p.lookAt(90+dir*180,28);
            Client.waitTick(breakTime*(3+stuck));
            p.lookAt(90+dir*180,pitchValue);
