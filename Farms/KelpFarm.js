...
const xChestCompactor = 6631;
const zChestCompactor = -5621;
...
function compact() { //Go to the compactor, put things in the chest and hit the furnace with the stick
    ...
    for (const slot of slots) {
        ...
        if (item === "minecraft:kelp") {
            if (lastCompact) {
                while (chestFull()) {
                    Client.waitTick(20); //Wait until you empty your inv if the chest is full
                }
            }
            if (!chestFull(inv)) { //Put the kelp if there's room in the chest
                inv.quick(slot);
                Client.waitTick();
            }
        }
